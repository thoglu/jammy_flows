...
def check_conditional_pdf(num_kde, plot=False, index=0, dimensionality=3):
    ...
    exponents=torch.rand( (dimensionality, num_kde, 2) , dtype=torch.double)*2+0.1
    ...
    skew_signs=pdf.layer_list[0][0].skew_signs

    total_cdf=pdf.layer_list[0][0].logistic_kernel_log_cdf(x, means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
    total_sf=pdf.layer_list[0][0].logistic_kernel_log_sf(x, means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
    #total_pdf=pdf.layer_list[0][0].logistic_kernel_log_pdf((x-0.5*bw).reshape(-1,1), means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
    total_pdf_right=pdf.layer_list[0][0].logistic_kernel_log_pdf((x), means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
...
def check_skewness(num_kde, plot=False, index=0):
    ...
    exponents=torch.rand( (1, num_kde, 1) , dtype=torch.double)*2+0.1
    ...
    total_cdf=pdf.layer_list[0][0].logistic_kernel_log_cdf(x.reshape(-1,1), means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
    total_sf=pdf.layer_list[0][0].logistic_kernel_log_sf(x.reshape(-1,1), means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
    total_pdf=pdf.layer_list[0][0].logistic_kernel_log_pdf((x-0.5*bw).reshape(-1,1), means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()
    total_pdf_right=pdf.layer_list[0][0].logistic_kernel_log_pdf((x+0.5*bw).reshape(-1,1), means, log_widths, log_norms, exponents, skew_signs).squeeze(-1).exp().numpy()


    numerical_cdf=numpy.cumsum(total_pdf)*bw
    numerical_sf=(numpy.cumsum(total_pdf_right[::-1])*bw)[::-1]

    assert( numpy.fabs(total_cdf-numerical_cdf).max() <1e-4) , ("MAX DIFF CDF num/exact: ", numpy.fabs(total_cdf-numerical_cdf).max())
    ...
    if(plot):   
        ...
        for cur_kde in range(num_kde):
            ...
            cur_pdf=pdf.layer_list[0][0].logistic_kernel_log_pdf((x-0.5*bw).reshape(-1,1), means[:,cur_kde:cur_kde+1,:], log_widths[:,cur_kde:cur_kde+1,:], log_norms[:,cur_kde:cur_kde+1,:], exponents[:,cur_kde:cur_kde+1,:],skew_signs[:,cur_kde:cur_kde+1,:]).squeeze(-1).exp().numpy()
            numerical_total_pdf+=cur_pdf*log_norms[0,cur_kde,0].exp().numpy()
            cur_sign=skew_signs[0,cur_kde, 0].numpy()
            cur_exponent=exponents[0,cur_kde,0].numpy()
            ax2.plot(x, cur_pdf, label="weighted pdf %.2d (%d/%.1f)" % (cur_kde, cur_sign,cur_exponent))
